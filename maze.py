...
from tableRoom import tableRoom
...
totalPathRandom = []                               # Includes ALL paths
totalPathLearning = []                              # Includes ALL paths
totalAveragePathRandom = []                         # Includes average paths
totalAveragePathLearning = []                       # Includes average paths
...
def moveAgent(agent, dict):
    ...
    # Filling in perception vector
    for pair in itertools.combinations(dict[currRoom], 2):
        agent.perceptionVector.append(max(pair[0].pres_smell, pair[1].pres_smell))
    ...
    if currRoom.pres_wall == 1 or currRoom.pres_hole == 1:
        moveTo = agent.path[-2]
    else:
    ...
    if agent.currentLocation.pres_monster > 0:
        ...
        if agent.currentLocation.isNormal:
            if tableRoomsNormal:
                currTableRoom = random.choice(tableRoomsNormal)
                currTableRoom.deathCount += 1
            reinforcementAgentTableNormal[tuple(agent.perceptionVector)] = tableRoomsNormal
        else:
            if tableRoomsBorder:
                currTableRoom = random.choice(tableRoomsBorder)
                currTableRoom.deathCount += 1
            reinforcementAgentTableBorder[tuple(agent.perceptionVector)] = tableRoomsBorder
...
lock1 = [0]
def moveTrainedAgent(agent, dict, sigma, omega):

    del agent.perceptionVector[:]

    if lock[0] == 0:
        currRoom = agent.currentLocation
        newlist = dict[currRoom]
        newlist.sort(key=lambda x: x.pres_smell)
        moveTo = newlist[0]
        agent.destination = moveTo
        agent.action = "Move to " + unicode(moveTo.id)
        lock[0] = 1
    ...
    if currRoom.isNormal:
        moveTo = findVector(agent.perceptionVector, reinforcementAgentTableNormal)
    else:
        moveTo = findVector(agent.perceptionVector, reinforcementAgentTableBorder)
    ...
    if currRoom.pres_wall == 1 or currRoom.pres_hole == 1:
        ...
        newlist = dict[currRoom]
        newlist.sort(key=lambda x: x.pres_smell)
        currSmell = newlist[0].pres_smell
        index = None
        for i in range(len(newlist)):
            if newlist[i].pres_smell > currSmell:
                index = i
                break
        newlist1 = newlist[:index]
        moveTo = random.choice(newlist1)
...
def final_output_with_cycles(dict, sigma, omega, file, agentType):
    ...
    if agentType == "random":
        ...
        while agent.isAlive and agent.goldPicked == 0:
            ...
            print_one_room(dict, agent.currentLocation)
            output(file)
        totalPathRandom.append(len(agent.totalPath))

    elif agentType == "learning":
        while agent.isAlive and agent.goldPicked == 0:
            tick(dict, sigma, omega)
            moveTrainedAgent(agent, dict, sigma, omega)
            print_one_room(dict, agent.currentLocation)
            output(file)
        totalPathLearning.append(len(agent.totalPath))
...
def main_input():
    while (True):
        ...
        if sinp[0].lower() == "initMaze".lower():
            ...
            file.truncate(0)
            fillTableRoomList(p, k)
            print ("###################################################################################################################")
            print ("Moving Random agent")
            print ("###################################################################################################################")
            ...
            for i in range(1, 10001):
                ...
                final_output_with_cycles(new_dict, sigma, omega, file, "random")
                resetGlobals()

            print ("###################################################################################################################")
            print ("Moving Learning agent")
            print ("###################################################################################################################")
            ...
            for i in range(1, 10001):
                ...
                allocate_wind(new_dict, sigma, omega)
                allocate_smell(new_dict, sigma, omega)
                final_output_with_cycles(new_dict, sigma, omega, file, "learning")
                resetGlobals()
            break
            ...
            for i in range(1, 10001):
                ...
                final_output_with_cycles(main_dict, sigma, omega, file, "random")
                resetGlobals()
            print ("###################################################################################################################")
            print ("Moving Learning agent")
            print ("###################################################################################################################")
            ...
            for i in range(1, 10001):
                ...
                allocate_smell(main_dict, sigma, omega)
                allocate_wind(main_dict, sigma, omega)
                final_output_with_cycles(main_dict, sigma, omega, file, "learning")
                resetGlobals()
            break
        else:
...
reinforcementAgentTableNormal = {}
reinforcementAgentTableBorder = {}
reinforcementMosterTableNormal = {}
reinforcementMosterTableNormal = {}
tableRoomsNormal = []                                     # Global list of table rooms (normal rooms)
tableRoomsBorder = []                                     # Global list of table rooms (border rooms)

# ADD THIS FUNCTION
def fillTableRoomList(p, k):
    for i in range(1, p + 1):
        tableRoomsNormal.append(tableRoom(i))
    for i in range(1, k + 1):
        tableRoomsBorder.append(tableRoom(i))


def fillTables(p, k, vector):

    lst1, lst2 = [], []
    for i in range(1, p + 1):
        lst1.append(tableRoom(i))
    for i in range(1, k + 1):
        lst2.append(tableRoom(i))

    reinforcementAgentTableNormal[tuple(lst1)] = None

def findVector(vector, table):
    if lock1[0] == 0:
        return None
    if tuple(vector) in table.keys():
        tableRooms = table[tuple(vector)]
        tableRooms.sort(key=lambda x: x.deathCount)

        return tableRooms[0]
    return None

# Taken from https://stackoverflow.com/a/7716358/9901274
def mean(numbers):
...
for i in range(100, 10001, 100):
    el = totalPathLearning[prev:i+1]
    totalAveragePathLearning.append(mean(el))
    totalAveragePathRandom.append(mean(totalPathRandom[prev:i+1]))
...
major_ticks = np.arange(0, 10001, 1000)
minor_ticks = np.arange(0, 10001, 200)
...
plt.plot(range(1, 10001, 100), totalAveragePathRandom, label = "Random moving agent", marker='o', linestyle='--', markersize = 3)
plt.plot(range(1, 10001, 100), totalAveragePathLearning, label = "Markov agent",marker='o', linestyle='--', markersize = 3)
...
# initmaze 10 5 3 5 4 0 0 0 3 2 0.5
